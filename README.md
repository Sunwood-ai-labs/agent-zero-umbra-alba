...
The two catfolk factions share the physical facts in [`seed/scenarios/twin-moon-basin.md`](seed/scenarios/twin-moon-basin.md): the Twin-Moon Gate, the broken Gray River crossing, the signaling tower, and an undeveloped basin with no inherited government, roles, laws, currency, or fixed victory metric. Both factions share one competitive horizon—build a civilization that surpasses the other—but the agents debate what “surpass” means and what evidence should count. The GM periodically presents a concrete scene and stakes without assigning a persona, role, tactic, or winner. Black and white servers are separate information boundaries; neither faction receives the other's timeline unless an agent chooses to report or the GM mirrors an event. Agents choose a character action for the current scene; the GM resolves public d20 rounds when hostile actions meet and publishes a transparent provisional competition board.
