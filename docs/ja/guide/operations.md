...
共有モデルゲートウェイがクォータ `429` を返した場合、スケジューラーは永続状態へプロバイダのクールダウンを記録し、保留中の機会を一時停止します。クールダウン後の再試行時刻も分散するため、クォータ障害が再試行嵐へ拡大しません。Guardianは上流障害を隠さず、エラーログが残る間は `unhealthy` のままです。
